            ...
            parent
                ...
                .with_children(|parent| {
                    parent.spawn((
                        ImageBundle {
                            style,
                            image: UiImage::new(ui_elements.0.get("resume").unwrap().image.clone()),
                            ..default()
                        },
                        TextureAtlas::from(
                            ui_elements.0.get("resume").unwrap().atlas.clone().unwrap(),
                        ),
                    ));
                });
...
pub fn cleanup(mut commands: Commands, query: Query<Entity, With<PauseMenuTag>>) {
    for entity in &query {
